...
# 2. Define target string to replace
target = '''    def get_kpi_summary(self, month: str = None) -> Dict[str, float]:
...
# 3. Define replacement string
replacement = '''    def get_kpi_summary(self, month: str = None, start_date: str = None, end_date: str = None) -> Dict[str, float]:
        """إحصائيات (رصيد، دخل، مصاريف) للـ Dashboard"""
...
if target in content:
    ...
    with open(filepath, 'w', encoding='windows-1256') as f:
        f.write(new_content)
    print("✅ Successfully updated get_kpi_summary in database.py!")
else:
    # Let's try matching with normalized line endings
    content_norm = content.replace('\r\n', '\n')
    target_norm = target.replace('\r\n', '\n')
    if target_norm in content_norm:
        new_content_norm = content_norm.replace(target_norm, replacement)
        with open(filepath, 'w', encoding='windows-1256', newline='\r\n') as f:
            f.write(new_content_norm)
        print("✅ Successfully updated get_kpi_summary in database.py (normalized line endings)!")
    else:
        print("❌ Error: Target string not found in database.py")
